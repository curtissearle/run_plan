...
const formSchema = z.object({
  ...
  customRaceDistance: z.number().positive("Distance must be positive").optional(),
  trainingDays: z.array(trainingDaySchema).min(1, "Select at least one training day"),
}).refine((data) => {
...
const runTypes: Run["type"][] = ["Easy", "Long", "Interval", "Tempo", "Strength"];
...
export default function InputForm({ onSubmit, initialValues }: InputFormProps) {
  ...
  const handleDayChange = (checked: boolean, day: string) => {
    ...
    if (checked) {
      form.setValue("trainingDays", [...currentDays, { day, runType: "Easy" }]);
    } else {
      form.setValue("trainingDays", currentDays.filter((d) => d.day !== day));
    }
    ...
    >
      ...
      <div>
        ...
        <div className="flex flex-col space-y-4">
          {daysOfWeek.map((day) => {
            const isChecked = watchedTrainingDays.some((d) => d.day === day);
            return (
              <div key={day} className="flex items-center space-x-4">
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor={day} className="ml-3 block text-sm font-medium text-gray-800 w-12">
                    {day}
                  </label>
                </div>
                {isChecked && (
                  <select
                    {...form.register(`trainingDays.${watchedTrainingDays.findIndex(d => d.day === day)}.runType`)}
                    className="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm text-black"
                  >
                    {runTypes.map((type) => (
